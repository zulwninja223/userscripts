...
const Utils = {
    ...
    getItemId(element) {
        const idClass = Array.from(element.classList).find(cls => cls.startsWith('item-id-'));
        if (idClass) {
...
class EnhanceManager {
    ...
    saveConfig() {
        ...
        this.config = {
            itemId: Utils.isNI() ? Utils.getItemId(this.itemSlot.firstChild) : (this.itemSlot.firstChild?.id?.substr(4) || 0),
...
function startManager() {
    ...
    if(Utils.isNI()){
        Utils.wrapFunction(window.Engine.items.afterUpdate, window.Engine.items, "afterUpdate", enhanceManager)
    } else {
...
if (Utils.isNI())
    window.API.addCallbackToEvent(window.Engine.apiData.AFTER_INTERFACE_START, startManager);
else
